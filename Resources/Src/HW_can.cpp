...
void HAL_CAN_RxFifo0MsgPendingCallback(CAN_HandleTypeDef *hcan) {
  if (hcan == &hcan1) {
        ...
        HAL_OK) // 获得接收到的数据头和数据
    {
      ...
      if (rx_header1.StdId == 0x1FF) { // 帧头校验
        int16_t temp = 0;
        temp = (int16_t)(((uint16_t)can1_rx_data[0]<<8) | (uint16_t)can1_rx_data[1]);
        rc_rv__ = ((float)temp)/1000.0f;
        temp = (int16_t)(((uint16_t)can1_rx_data[2]<<8) | (uint16_t)can1_rx_data[3]);
        rc_rh__ = ((float)temp)/1000.0f;                                 // 校验通过进行具体数据处理
      }
    }
  }
